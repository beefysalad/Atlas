...
export class InventoryService {
  ...
  async deleteInventoryItem(itemId: string): Promise<void> {
    ...
    const movementCount =
      await this.inventoryRepository.countMovementsForItem(itemId)
  ...
  async updateInventoryMovement(
    ...
    if (existingMovement.itemId === input.itemId) {
      ...
      const nextTargetItemQuantity =
        nextItem.onHandQuantity + nextSignedQuantity
  ...
  private getSignedQuantity(
    type:
      ...
      | UpdateInventoryMovementRequest["type"],
    quantity: number
  ): number {
    if (STOCK_INCREASE_TYPES.has(type)) {
      return quantity
